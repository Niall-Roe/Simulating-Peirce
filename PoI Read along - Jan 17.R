...
server <- function(input, output, session) {
  ...
  # - Add grid visualizations to Example 1
  ...
  output$ex1_grid <- renderPlot({
    ...
    for (i in 1:n_suits) {
      for (j in 1:n_ranks) {
        suit <- suits[i]
        rank <- ranks[j]
        matches <- card_matches(suit, rank, target)
        if (matches) count_match <- count_match + 1
        
        col <- if (matches) "#deebf7" else "white"
    ...
    prob <- count_match / total_cards
    text(n_ranks/2, -0.3,
         paste0("Green cells match consequent: ", count_match, "/", total_cards,
                " = P(A → C) = ", round(prob, 4)),
         cex = 0.9)
  })
